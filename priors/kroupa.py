...
class Kroupa(Prior):
    ...
    def process(self, **kwargs):
        """Process module."""
        #print('kroupa kwargs', kwargs)
        kroupainput = kwargs['kroupainput']

        self._norm_factor = 1./self.Xtotal(Kroupa, 100, 1)
        #print ('norm_factor', self._norm_factor)
        mass = self.Mtotal(Kroupa, kroupainput, self._norm_factor)
        return mass
    ...
    def Xtotal(self, m, k):
        ...
        if m < 0.5:
            return ((k/0.7 * 0.08**0.3 * (0.08**0.7 - 0.01**0.7)) +
                    (k/-0.3) * 0.08**1.3 * (m**(-0.3) - 0.08**(-0.3)))
        return ((k/0.7 * 0.08**0.3 * (0.08**0.7 - 0.01**0.7)) +
                (k/-0.3) * 0.08**1.3 * (0.5**(-0.3) - 0.08**(-0.3)) +
                (k/-1.3) * 0.08**1.3 * (0.5 * m**(-1.3) - 0.5**(-0.3)))
    ...
    def Mtotal(self, f, k):
        ...
        """
        if f < self.Xtotal(Kroupa, 0.08, k):
            return (f * (0.7)/k * 0.08**(-0.3) + 0.01**0.7)**(1/0.7)
        elif f < self.Xtotal(Kroupa, 0.5, k):
            return (((f - (k/0.7 * 0.08**0.3 * (0.08**0.7 - 0.01**0.7))) *
                     (-0.3)/k * 0.08**(-1.3) + 0.08**(-0.3))**(1/-0.3))
        return (((f - (k/-0.3) * 0.08**1.3 * (0.5**(-0.3) - 0.08**(-0.3)) -
                 (k/0.7 * 0.08**0.3 * (0.08**0.7 - 0.01**0.7))) * -1.3/k *
                0.5**(-2.3) * (6.25)**1.3 + 0.5**(-1.3))**(1/-1.3))
