...
then
    ...
    RUNNER=python
    TRUNNER=python
    if [ "$1" = -c ]; then echo "travis_fold:start:FIT Fitting test data"; fi
    ...
    if [ "$1" = -c ]; then echo "travis_fold:end:FIT Fitting test data done"; fi
    if [ "$1" = -c ]; then echo "travis_fold:start:GEN Generating random models"; fi
    $RUNNER -m mosfit --travis -i 0
    $RUNNER -m mosfit -i 0 -m default -P parameters_test.json
    $TRUNNER test.py
    if [ "$1" = -c ]; then echo "travis_fold:end:GEN Generating random models done"; fi
    if [ "$1" = -c ]; then echo "travis_fold:start:JUP Testing Jupyter notebooks"; fi
    if [ "$1" = -c ]; then echo "travis_fold:end:JUP Testing Jupyter notebooks"; fi
    coverage combine
fi
