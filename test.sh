...
$RUNNER -m mosfit -e mosfit/tests/LSQ12dlf.json --test -i 2 --no-fracking -m rprocess --variance-for-each band --offline
if [ "$1" = -c ]; then
    $RUNNER -m mosfit -e SN2007bg --test -i 1 --no-fracking -m ic --language ru
else
    $RUNNER -m mosfit -e SN2007bg --test -i 1 --no-fracking -m ic
fi
echo -ne 'y\n\n4\n2\n4\n4\nu\n\n\n' | $RUNNER -m mosfit -m slsn -e mosfit/tests/PTF10hgi.txt -i 1 --no-write
