...
class BlackbodyCutoff(SED):
    ...
    def process(self, **kwargs):
        ...
        self._cutoff_wavelength_slope = kwargs[self.key('cutoff_wavelength_slope')]
        self._cutoff_wavelength_stop = kwargs[self.key('cutoff_wavelength_stop')]
        self._times = np.array(kwargs['rest_times'])
        xc = self.X_CONST  # noqa: F841
        fc = self.FLUX_CONST
        cc = self.C_CONST
        ac = ANG_CGS
        cwave_ac_0 = self._cutoff_wavelength * ac
        cwave_ac2_0 = cwave_ac_0 * cwave_ac_0
        cwave_ac3_0 = cwave_ac2_0 * cwave_ac_0  # noqa: F841
        ...
        for li, lum in enumerate(self._luminosities):
            ...
            # tpi = tp[li]
            # rp2i = rp2[li]
            cwave_ac = cwave_ac_0 + self._times[li] * self._cutoff_wavelength_slope
            if cwave_ac > self._cutoff_wavelength_stop:
                cwave_ac = self._cutoff_wavelength_stop
