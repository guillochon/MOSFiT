...
class CSM(Engine):
    ...
    def process(self, **kwargs):
        ...
        self._g_n = 1.0 /(4.0 * np.pi * (self._n - self._delta)) * (2.0 * (5.0 - self._delta) * (self._n - 5.0) * self._Esn)\
                ** ((self._n - 3.) / 2.0)/((3.0 - self._delta) * (self._n - 3.0) * self._mejecta) \
                ** ((self._n - 5.0) / 2.0)  # g**n is scaling parameter for ejecta density profile
        ...
        self._Mcsm_th = 4.0 * np.pi * self._q / (3.0 - self._s) * (self._Rph ** (3.0 - self._s) - self._R0
                        ** (3.0 - self._s))   # mass of the optically thick CSM (tau > 2/3)


        self._t_FS = abs((3.0 - self._s) * self._q ** ((3.0 - self._n)/(self._n - self._s)) * (self._A * self._g_n)
                         **((self._s - 3.0) / (self._n - self._s))/(4.0 * np.pi * self._Bf ** (3.0 -self._s)))\
                     ** ((self._n - self._s) / ((self._n - 3.0) * (3.0 - self._s))) * (self._Mcsm_th)\
                     **((self._n - self._s) / ((self._n - 3.0) * (3.0 - self._s)))    # time at which shock breaks out of optically thick CSM - forward shock power input then terminates

        self._t_RS =  (self._vph / (self._Br * (self._A * self._g_n / self._q) ** (1.0 / (self._n - self._s))) *
                       (1.0 - (3.0 - self._n) * self._mejecta / (4.0 * np.pi * self._vph ** (3.0 - self._n)
        ...
        luminosities = [2.0 * np.pi / (self._n - self._s)**3 * self._g_n ** ((5.0 - self._s)/(self._n - self._s)) * \
                    self._q ** ((self._n - 5.0)/ (self._n - self._s)) * (self._n - 3.0) ** 2 * (self._n - 5.0) * \
                    self._Bf ** (5.0 - self._s) * self._A ** ((5.0 - self._s) / (self._n - self._s)) * (t *
                     86400. + self._ti) ** ((2.0 * self._n + 6.0 * self._s - self._n * self._s - 15.) / (self._n - self._s)) \
                       * ((self._t_FS- t * 86400.)>0) + 2.0 * np.pi * (self._A * self._g_n / self._q) ** ((5.0 - self._n) / (self._n - self._s)) * \
                    self._Br ** (5.0 - self._n) * self._g_n * ((3.0 - self._s) / (self._n - self._s)) ** 3 *\
                    (t * 86400. + self._ti) ** ((2.0 * self._n + 6.0 * self._s - self._n * self._s - 15.0) /
