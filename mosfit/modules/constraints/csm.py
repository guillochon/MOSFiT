...
class CSMConstaints(Constraint):
    ...
    def process(self, **kwargs):
        ...
        self._s = kwargs['s']
        self._mcsm = kwargs['csm'] * M_SUN_CGS
        ...
        self._gn = (1.0 / (4.0 * np.pi * (self._n - self._delta)) * (
            2.0 * (5.0 - self._delta) * (self._n - 5.0) * self._Esn)**(
                (self._n - 3.) / 2.0) / (
                    (3.0 - self._delta) * (n - 3.0) * self._mejecta)**(
                        (self._n - 5.0) / 2.0)
                     )  # g**n is scaling parameter for ejecta density profile
        self._q = self._rho * self._r0**self._s
        self._R_csm = ((3.-s)/(4.*np.pi*self._q)*self._mcsm+self._r0**(3.-self._s))**(1./(3.-self._s))
        self._R_ph = (R_csm**(1.-self._s) - 2. * (1.-self._s)/(3.*self._kappa*q))**(1./(1.-self._s))
        self._Mcsm_th = 4.0 * np.pi * q / (3.0 - self._s) * (
            R_ph**(3.0 - self._s) - self._r0**(3.0 - self._s))  # mass of the optically thick CSM (tau > 2/3)
        self._tf = ((R_csm-self._r0)/beta_f/(A*gn/q)**(1./(self._n-self._s)))**((self._n-self._s)/(self._n-3))
        self._td = np.sqrt(2.*self._kappa*Mcsm_th/(self._vejecta*13.7*3.e10))

        #Constraint 1: R0<= Rph <= Rcsm
        if (R_csm < R_ph) | (self._r0 > R_ph):
            score_modifier = LIKELIHOOD_FLOOR
        ...
        return {'score_modifier':score_modifier}
