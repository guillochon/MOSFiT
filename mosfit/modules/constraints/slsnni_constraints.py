...
from mosfit.constants import DAY_CGS, KM_CGS, M_SUN_CGS, M_P_CGS, MEV_CGS
...
class SLSNConstraints(Constraint):
    ...
    def process(self, **kwargs):
        ...
        if self._Ek > (self._Ep + self._Emax) - E_rad + self._neutrino_energy:
            self._score_modifier += -(
                self._Ek - ((self._Ep + self._Emax) - E_rad + self._neutrino_energy)) ** 2 / (
